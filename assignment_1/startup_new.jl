...
body_two = findbody(mechanism, "Link two prime")
point_one = Point3D(default_frame(body), 0., 0.1, 0.1)
...
##############################################################
function jacobian_transpose_ik_child!(state::MechanismState,
                               body_two::RigidBody,
                               point_one::Point3D,
                               desired::Point3D;
                               α=0.1,
    ...
    # Compute the joint path from world to our target body
    p = path(mechanism, root_body(mechanism), body_two)
    # Allocate the point jacobian (we'll update this in-place later)
    Jp = point_jacobian(state, p, transform(state, point_one, world))
...
desired_tip_location = Point3D(root_frame(mechanism), 0.5, 2, 0.8)
...
###################################################################
jacobian_transpose_ik!(state, body_two, point_one, desired_tip_location_two)
set_configuration!(vis, configuration(state))
###################################################################
...
###################################################################
transform(state, point_one, root_frame(mechanism))
...
for x in range(-1, stop=1, length=100)
    ...
    #################################################
    jacobian_transpose_ik_child!(state, body_two, point_one, desired)
    ##################################################
